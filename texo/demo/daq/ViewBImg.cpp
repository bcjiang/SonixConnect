...
bool ViewBImg::init(int scanLines, int samples)
{
...
void ViewBImg::convert2RGB(unsigned char* buffer, unsigned char* bufferImg)
{
...
bool ViewBImg::setImgData(unsigned char* data)
{
...
void ViewBImg::assignBuffer(int w, int h)
{
    if (m_image)
    {
        delete m_image;
...
void ViewBImg::allocateBuffer(int sz)
{
    if (m_buffer)
    {
        delete[] m_buffer;
...
void ViewBImg::createRGBColorMap()
{
    // create RGB colormap
    if (m_colorMap)
    {
